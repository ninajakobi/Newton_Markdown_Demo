...
# borrow the Bruce County base map
bruce_map <- brick(BC)
bruce_crs <- st_crs(bruce_map)

# list of places Rachel has gone camping:
camping <- tibble(latitude = c(45.23090243980543, 44.41411829694316, 44.53774239289965, 43.24856359980997, 44.29964881461152),
                    longitude = c(-81.52241785248054, -81.45530851742954, -80.35132345391561, -81.82370140476118, -81.58808660209003),
                    park = c("bruce_peninsula", "macgregor_point", "craigleith", "pinery", "inverhuron"),
                    park_type = c("national", "provincial", "provincial", "provincial", "provincial")) %>% 
    st_as_sf(coords = c("longitude", "latitude")) %>%
  st_set_crs(bruce_crs)

# plot on map
ggRGB(bruce_map, r = 1, g = 2, b = 3) +
  geom_sf(data = camping, aes(fill = park_type), size = 5, shape = 21) +
  scale_fill_manual(values = c("green4", "dodgerblue4"), 
                    breaks = c("National", "Provincial"), name = "Park Type")+
  theme_void(base_size = 15) +
  theme(legend.position = c(0.8, 0.8))
